...
hproto.resolveImportPaths = function() {
  if(this.hasResolvedImportPaths) {
    ...
    function resolveImportPath(path){
      var strippedPath = (path + "").replace(/<link rel=\"import\" href=\"[^\"]+\">/g, ''),
          resolvedPath = strippedPath;
      resolvedPath = resolvedPath.replace(/<link rel=\"import\" href=\"[^\"]+\">/g, '');

      try {
        resolvedPath = theContext.html(resolvedPath);
        if(resolvedPath === '') {
        ...
        console.error("Can't resolve html path '" + resolvedPath + "' in '" + actual + "'");
        resolvedPath = strippedPath;
      }
      return "<link rel='import' href='" + resolvedPath + "'>";
    }

    //fix the import paths in the html file
    var regex = /<link rel=\"import\" href=\"[^\"]+\">/g
