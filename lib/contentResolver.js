...
function readContentSync(meta, isSubAssembly) {
  ...
  if(meta.assembled) {
    try {
      ...
      var fileList                  = utils.globListToList(meta.baseModulePath, assembly.files || []);
      ...
      try { contents += "// "+meta.baseModulePath+"\n"; } catch(ex) {}
      try { contents += "// "+assembly.files.join(", ")+"\n"; } catch(ex) {}
      try { contents += "// file: "+fileList.join("\n// file: ") + "\n\n"; } catch(ex) {}
      ...
      if (filesLength > 0) {
        ...
        // Append any files
        contents += appendFiles(meta.baseModulePath, filesLength, fileList);
