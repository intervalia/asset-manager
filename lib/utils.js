...
exports.globListToList = function(rootFolder, globList) {
  var temp, options, fileList = [];
  if (globList && Array.isArray(globList)) {
    try {
      temp = globList.join(",");
      if (temp.indexOf("*") > -1) {
        options = {
        ...
        glob(temp, options, function globAllCB(er, files){
          fileList = files;
        });
      }
      else {
